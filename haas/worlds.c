...
int hash_size;
{
    ...
    /* STUFF FOR THE POPULATION VIEWER. */
	...
	for (col = 0; col < ncols; col++){
	    ...
	    /* If the site produces b's, write out its coordinates. */
	    if (SITE_PRODUCES(WORLD_SITE(the_world, row, col), 1) > 0){
    ...
    /* STUFF FOR THE MUTATION VIEWER. */
    
    sprintf(mutation_file_name, "%ld.mutations", old_seed);
    mutation_file = Fopen(mutation_file_name, "w");
    fprintf(mutation_file, "%d\n%d\n", nrows, ncols);
    ...
    /* STUFF FOR THE RESOURCE VIEWER. */
    
    sprintf(resource_file_name, "%ld.resources", old_seed);
    resource_file = Fopen(resource_file_name, "w");
    fprintf(resource_file, "%d\n%d\n", nrows, ncols);
	...
	for (col = 0; col < ncols; col++){
	    ...
	    /* Write out the number of a's the site produces. */
	    fprintf(resource_file, "%d\n", SITE_PRODUCES(WORLD_SITE(the_world, row, col), 0));
	}
    }
