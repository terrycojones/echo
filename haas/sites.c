#ifndef lint
static char *rcsid = "$Header: /tmp_mnt/vida/disks/disk5/Users/terry/r/echo/RCS/sites.c,v 1.9 1992/11/16 07:05:45 terry Exp terry $";
#endif
...
SITE *site;
{
    ...
    /* We always make the agent pay for resource 'a'. */
    tax[0] = SITE_MAINTENANCE(site, 0);
    
    for (agent = SITE_NAGENTS(site) - 1; agent >= 0; agent--){

	/* Charge for resource 'b' with the given maintenance charge probability. */
	if (biased_coin_flip(p_maintenance)){
	    tax[1] = SITE_MAINTENANCE(site, 1);
	}
	else {
	    tax[1] = 0;
	}

	/* Now find out if they can pay. If so, charge them, if not, delete them. */
	if (can_pay_maintenance(tax, SITE_AGENT(site, agent))){
	    sub_resources(AGENT_RESERVE_LIST(SITE_AGENT(site, agent)), tax);
	    /* Just print a debug message if we're charging them b's. */
	    if (tax[1]){
...
SITE *site;
{
    ...
    register AGENT_POPULATION_SIZE nagents;
	...
	AGENT_HAS_MIGRATED(agents[agent]) = FALSE;
    }
...
SITE *site;
{
     ...
     *
     * For the Haas model, if the site produces b's, we give one to every
     * agent at the site that can pick them up.
     */
    ...
    /* Let everyone have some number of b's (see defaults.h) if this site produces them. */
    if (ntakers[1] && SITE_PRODUCES(site, 1)){
	shares[1] = DEFAULT_WATER_UPTAKE;
    }
    else {
	shares[1] = 0;
    }
    ...
    /*
     * Figure out how much each agent that wants resource 'a' will get. Integers!
     * Unlike resource 'b', this is based on how much is lying around at the site.
     */
    if (ntakers[0]){
	shares[0] = SITE_RESOURCE(site, 0) / ntakers[0];
    }
    else {
	shares[0] = 0;
    }
	...
	if (shares[resource]){
	    ...
	    /* Don't subtract from the site for resource 'b', which is "infinite". */
	    if (resource == 0){
     ...
     * There will be no remainder for resource 'b', that doesn't
     * make sense in the Haas model, since it is "infinite" and agents
     * always get some if they're at a site that produces b's.
     * Hence the nresources - 1 in the for loop.
     */
...
char *file;
{
	...
	*count = '\0';
	...
	if (count == newline){
	    ...
	    for (i = 0; i < n; i++){
		agent++;
		SITE_AGENT(new, agent) = copy_agent(SITE_AGENT(new, agent - 1), FALSE);
	    } 
	}
...
{
    /*
     * Self replicate the agents if they have been alive long enough.
    ...
    for (agent = 0; agent < nagents; agent++){
	if (AGENT_TIME_SINCE_REPLICATION(agents[agent]) >= SELF_REPLICATION_INTERVAL){
	    AGENT_CAN_SELF_REPLICATE(agents[agent]) = TRUE;
	    AGENT_TIME_SINCE_REPLICATION(agents[agent]) = 0;
	    nreplicators++;
	}
	else {
	    AGENT_CAN_SELF_REPLICATE(agents[agent]) = FALSE;
	    AGENT_TIME_SINCE_REPLICATION(agents[agent])++;
	}
    } 
...
#ifndef NO_MIGRATION
void
site_migrate_agents(site, probabilities)
SITE *site;
double *probabilities;
{
    /*
     * Migrate those agents according to the probability encoded
     * in the genome's migration bits. The bits are counted and this number
     * is divided by the total number of possible bits (NMIGRATION_BITS)
     * to give a probability for migration.
     *
     * We have to check to see if this agent has already migrated this
     * generation. If so, we don't move it again.
     */
    AGENT_POPULATION_SIZE agent;
    AGENT_POPULATION_SIZE nagents = SITE_NAGENTS(site);
    AGENT **agents = SITE_AGENTS(site);
    ...
    for (agent = 0; agent < nagents; agent++){
	if (AGENT_HAS_MIGRATED(agents[agent]) == FALSE){

	    register int ones_count = 0;
	    register int i;

	    for (i = 0; i < NMIGRATION_BITS; i++){
		if (AGENT_MIGRATION_BIT(agents[agent], i) == '1'){
		    ones_count++;
		}
	    }

	    if (biased_coin_flip(probabilities[ones_count])){
		AGENT_HAS_MIGRATED(agents[agent]) = TRUE;
...
int
site_migration_average(site)
SITE *site;
{
    AGENT_POPULATION_SIZE ones_count = 0;
    AGENT_POPULATION_SIZE nagents = SITE_NAGENTS(site);
    AGENT_POPULATION_SIZE agent;
    register AGENT **agents = SITE_AGENTS(site);
    ...
    return nagents ? ones_count / nagents : 0;
}
