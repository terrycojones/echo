#ifndef lint
static char *rcsid = "$Header: /disks/disk1/Users/terry/r/echo/s.orig/RCS/interaction.c,v 1.9 1994/02/07 23:16:04 terry Exp terry $";
#endif
...
AGENT *a2;
{
    ...
    if (update_combat == TRUE && update_on_testing == TRUE){
	AGENT_COMBAT_COND_USAGE(a1)++;
	AGENT_OFFENSE_TAG_USAGE(a2)++;
    }
...
AGENT *a2;
{
    ...
    if (update_trading == TRUE && update_on_testing == TRUE){
	AGENT_OFFENSE_TAG_USAGE(a1)++;
	AGENT_TRADE_COND_USAGE(a1)++;
	AGENT_OFFENSE_TAG_USAGE(a2)++;
	AGENT_TRADE_COND_USAGE(a2)++;
    }
...
AGENT *a2;
{
    ...
    if (update_mating == TRUE && update_on_testing == TRUE){
	AGENT_MATING_TAG_USAGE(a1)++;
	AGENT_MATING_COND_USAGE(a1)++;
	AGENT_MATING_TAG_USAGE(a2)++;
	AGENT_MATING_COND_USAGE(a2)++;
    }
...
FRACTION trade_fraction;
{
	...
	/*
	 * BEDAU: Increment the usage counters for the genes that were just involved.
	 * This is the offense and defense tags for both agents since we are doing
	 * (or did in combat-payoff) a two-way check.
	 */
	if (update_combat == TRUE && update_on_success == TRUE){
	    AGENT_OFFENSE_TAG_USAGE(agent1)++;
	    AGENT_DEFENSE_TAG_USAGE(agent1)++;
	    AGENT_OFFENSE_TAG_USAGE(agent2)++;
	    AGENT_DEFENSE_TAG_USAGE(agent2)++;
	}

	/*
	 * Compute the winner probabilistically.
	 */
	...
	if (will_trade(agent1, agent2)){
	    ...
	    /* BEDAU: Update the gene usage counters that were just used. */
	    if (update_trading == TRUE && update_on_success == TRUE){
		AGENT_OFFENSE_TAG_USAGE(agent1)++;
		AGENT_TRADE_COND_USAGE(agent1)++;
		AGENT_OFFENSE_TAG_USAGE(agent2)++;
		AGENT_TRADE_COND_USAGE(agent2)++;
	    }
	    ...
	    will_mate(agent1, agent2) == TRUE)
	{
	    ...
	    /* BEDAU: Update the gene usage counters that were just used. */
	    if (update_mating == TRUE && update_on_success == TRUE){
		AGENT_MATING_TAG_USAGE(agent1)++;
		AGENT_MATING_COND_USAGE(agent1)++;
		AGENT_MATING_TAG_USAGE(agent2)++;
		AGENT_MATING_COND_USAGE(agent2)++;
	    }
