...
STRING file;
{
	...
	set_chromosome(AGENT_CHROMOSOME(new, chromosome), AGENT_EDIT_CHROMOSOME(edit, chromosome), 0L);
    } 
...
AGENT *agent;
{
    /*
     * BEDAU:
     * The chars in the chromosomes, an '_' after each, the usage for each is
     * assumed to be 10 digits (this should be ample!), then an _ following each usage,
     * one char trading resource, one '_', uptake mask.
     */
    
    RESOURCE_SIZE genome_length = total_resources(AGENT_MAKEUP_LIST(agent)) +
	NCHROMOSOMES + (10 * NCHROMOSOMES) + NCHROMOSOMES + 1 + 1 + nresources;
	...
	if (AGENT_MAX_GENOME_LEN(agent) < genome_length){
	    ...
	    AGENT_MAX_GENOME_LEN(agent) = genome_length;
	}
    }
    else {
	AGENT_GENOME(agent) = Malloc(genome_length + 1);
	...
	/* BEDAU: added the usage counts to the sprintf below. */
	sprintf(AGENT_GENOME(agent), "%s_%ld_%s_%ld_%s_%ld_%s_%ld_%s_%ld_%s_%ld_%c_%s",
		AGENT_OFFENSE_TAG(agent), AGENT_OFFENSE_TAG_USAGE(agent),
		AGENT_DEFENSE_TAG(agent), AGENT_DEFENSE_TAG_USAGE(agent),
		AGENT_MATING_TAG(agent),  AGENT_MATING_TAG_USAGE(agent),
		AGENT_COMBAT_COND(agent), AGENT_COMBAT_COND_USAGE(agent),
		AGENT_TRADE_COND(agent),  AGENT_TRADE_COND_USAGE(agent),
		AGENT_MATING_COND(agent), AGENT_MATING_COND_USAGE(agent),
    ...
    AGENT_GENOME_LEN(agent) = strlen(AGENT_GENOME(agent));
...
SITE *site;
{
	...
	/* Create new chromosomes if the results are different. */
	
	if (strcmp(CHROMOSOME_STR(AGENT_CHROMOSOME(agent_1, chrom_1)), new_1)){
	    set_chromosome(AGENT_CHROMOSOME(agent_1, chrom_1), new_1, 0L);
	}
	    
	if (strcmp(CHROMOSOME_STR(AGENT_CHROMOSOME(agent_2, chrom_1)), new_2)){ 
	    set_chromosome(AGENT_CHROMOSOME(agent_2, chrom_1), new_2, 0L);
	}
	...
	else
	{
	    ...
	    if (strcmp(CHROMOSOME_STR(AGENT_CHROMOSOME(agent_1, chrom_1)), new_1_1)){
		set_chromosome(AGENT_CHROMOSOME(agent_1, chrom_1), new_1_1, 0L);
	    }
	    
	    if (strcmp(CHROMOSOME_STR(AGENT_CHROMOSOME(agent_1, chrom_2)), new_1_2)){
		set_chromosome(AGENT_CHROMOSOME(agent_1, chrom_2), new_1_2, 0L);
	    }
	    
	    if (strcmp(CHROMOSOME_STR(AGENT_CHROMOSOME(agent_2, chrom_1)), new_2_1)){
		set_chromosome(AGENT_CHROMOSOME(agent_2, chrom_1), new_2_1, 0L);
	    }
	    
	    if (strcmp(CHROMOSOME_STR(AGENT_CHROMOSOME(agent_2, chrom_2)), new_2_2)){
		set_chromosome(AGENT_CHROMOSOME(agent_2, chrom_2), new_2_2, 0L);
	    }
    ...
    /* BEDAU: Give the new agents a unique identifier. */
    AGENT_ID(agent_1) = total_agents++;
    AGENT_ID(agent_2) = total_agents++;
