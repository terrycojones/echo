#ifndef lint
static char *rcsid = "$Header: /disks/disk1/Users/terry/r/echo/s.orig/RCS/sites.c,v 1.11 1994/02/07 23:16:07 terry Exp terry $";
#endif
...
STRING file;
{
	...
	*tmp2 = '\0';
	...
	/* The representation of the genome is now pointed to by tmp. */
	for (i = 0; i < NCHROMOSOMES; i++){
	    ...
	    *underscore = '\0';
	    ...
	    usage = tmp = underscore + 1;

	    underscore = index(tmp, '_');

	    if (!underscore){
		popup_message("Chromosomes must be followed by usage and an underscore! (line %d, chromosome %d)",
			      line_num, i + 1);
	    }
	    ...
	    *underscore = '\0';
	    ...
	    set_chromosome(AGENT_CHROMOSOME(new, i), name, atol(usage));
	} 
