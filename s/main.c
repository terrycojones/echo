...
#ifndef NO_SPECIES_ABUNDANCE
char population_file_name[MAXPATHLEN];
...
#ifndef NO_SPECIES_ABUNDANCE
	    sprintf(population_file_name, "%ld.population", old_seed);
	    population_file = Fopen(population_file_name, "w");
	    site_list_agents(WORLD_SITE(the_world, 0, 0), FALSE, population_file);
	    Fclose(population_file);

	    sprintf(command, "cluster -d - > %ld.cluster", old_seed);
