#ifndef lint
static char *rcsid = "$Header: /tmp_mnt/winnetou/disks/disk3/Users/terry/r/echo/s/RCS/running.c,v 1.9 1995/04/19 02:33:43 terry Exp terry $";
#endif
...
int hash_size;
{
	...
	while (WORLD_RUNNING(the_world) == TRUE && WORLD_GENERATION(the_world) <= WORLD_HALT_GENERATION(the_world)){
		...
		for (col = 0; col < ncols; col++){
		    ...
		    /*
		     * The number of agents at a site may have changed,
		     * so change the number of interactions.
		     */
		    SITE_INTERACTIONS(site) = (AGENT_POPULATION_SIZE)
					      (WORLD_C(the_world) * (double) SITE_NAGENTS(site));
		} 
	    } 
#endif
...
#if 0  /* Mutation is now done only on self-replication. */
#ifndef NO_MUTATION
